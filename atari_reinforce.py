...
# use this for Atari games other than Pong
...
# CHANGES FOR BREAKOUT TRIED:
#   baseline across 10 games instead of 1
#   scaled reward by 1/50
#   half frame values before frame differencing.
...
arg_parser.add_argument("env",
                        nargs='?',
                        default="SpaceInvaders-v5",
                        help="See Gymnasium documentation for available Atari"
                            + " environments")
...
OPTIMISER = args.opt
print(f"Argument 1: {environment_name}")
print(f"Argument 2: {num_episodes}")
print(f"Argument 3: {learning_rate}")
print(f"Argument 4: {NET_ARCHITECTURE}")
print(f"Argument 5: {OPTIMISER}")
...
class CropObservation(ObservationWrapper):
    def __init__(self, env, crop_top, crop_bottom):
        ...
        self.observation_space = gym.spaces.Box(low = 0, high = 255, shape=(temp_shape[0]-crop_top-crop_bottom, temp_shape[1]))
...
def run_stochastic_policy(policy_network, observation):
    ...
    action = np.random.choice(range(num_outputs), p=action_probabilities)
    #print("action: ", action)
    assert(action<num_outputs)
    return action
...
env_silent = gym.make(f"ALE/{environment_name}", render_mode=None)

## pre processing
if not karp_prepro:    
    env_silent = GrayScaleObservation(env_silent)
    env_silent = ResizeObservation(env_silent, shape=(105, 80))
    env_silent = CropObservation(env_silent, crop_top=16, crop_bottom = 9)
    #transform range form [0, 255 ] to [-1, 1] like kp_prepro does
    #env_silent = TransformObservation(env_silent, lambda obs: ((obs/255.0)*2) - 1)
...
new_dir_name = ("results/" + environment_name + datetime.now().strftime("%Y%m%d_%H%M") + "_" 
                + NET_ARCHITECTURE + "_DF_" + str(discount_factor) + "_LR_" 
                + str(learning_rate) + "_OPT_" + OPTIMISER + "_NSR_SCALED_HALF")
os.makedirs(new_dir_name)

plt.ion()
plt.figure()

# need more memory for other games, use cpu
with tf.device("/cpu:0"):
    for episode in range(num_episodes):
        ...
        # there is a bug where episodes go fo an indefinite amount of time, 
        # terminate those.
        ...
        if episode_rewards_sum == max_reward_so_far:
            ...
            keras.models.save_model(policy_network, new_dir_name 
                                    + f"/{environment_name}_BEST_{episode_rewards_sum:.2f}" 
        ...
        if episode % 500 == 0 and episode != 0 :
            ...
            keras.models.save_model(policy_network, new_dir_name 
                                    + f"/{environment_name}_FIT{curr_average_fitness:.2f}_DF" 
                                    + str(discount_factor) 
                                    + "_LR" + str(learning_rate) 
                                    + "_EPS" + str(episode)
                                    + "_KP_Conv.h5",save_format='h5')
        ...
        if episode % 250 == 0 and episode != 0 :
            ...
            if curr_average_fitness > best_avg_fitness + 0.1:
                print(f"{curr_average_fitness} greater than {best_avg_fitness + 0.1}")
                print("Continuing")
        ...
        if len(mean_discounted_reward_history)>11:
            hist_len=min(len(mean_discounted_reward_history),10)
            ...
            reward_scaler=1/(np.std(arr)+1)/20 #This attempts to rescale the rewards so that they are closer to being in the range [-1,1], which 
...
if karp_prepro:
    ...
    keras.models.save_model(policy_network, new_dir_name 
                                + f"/{environment_name}_FIT{final_average_fitness:.2f}_DF" 
                                + str(discount_factor) 
                                + "_LR" + str(learning_rate) 
                                + "_EPS" + str(num_episodes)
                                + "_KP_Conv_NSR.h5",save_format='h5')
else:
    final_average_fitness=np.array(reward_history[-50:]).mean()
    print("Finished Training.  Final average fitness",final_average_fitness)
    # Save the current model into a local folder
    keras.models.save_model(policy_network, new_dir_name + "/Model_"+environment_name+"_DF" 
                            + str(discount_factor) + "_LR"+str(learning_rate) 
                            + "_EPS"+str(num_episodes)+"_Conv.h5",save_format='h5')
