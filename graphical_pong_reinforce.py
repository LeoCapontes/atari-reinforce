...
# Use this for real time reward plotting and occasional playback
...
env_graphical.action_space = [1, 2, 3]
env=env_silent
...
ax.axis([0,num_episodes+1,-21,21])
plt.ylabel('Reward')
...
for episode in range(num_episodes):
    if episode%100==0 and episode!=0: 
        # We only want to show Pong every 100 eps (otherwise training gets too slow)
        env=env_graphical
    else:
        env=env_silent
